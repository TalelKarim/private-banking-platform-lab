...
JENKINS_PORTFOLIO_JOB=${JENKINS_PORTFOLIO_JOB:-portfolio-java-deploy}
JENKINS_BUILD_TIMEOUT_SECONDS=${JENKINS_PORTFOLIO_BUILD_TIMEOUT_SECONDS:-1800}
...
LOCAL_AUTH_FILE=$(mktemp /tmp/private-banking-jenkins-portfolio-auth.XXXXXX)
KNOWN_HOSTS=$(mktemp /tmp/private-banking-jenkins-portfolio-known-hosts.XXXXXX)
REMOTE_AUTH_FILE="/tmp/private-banking-jenkins-portfolio-auth.$$"
cleanup() {
...
printf 'Triggering Jenkins application deployment job %s...\n' "$JENKINS_PORTFOLIO_JOB"
...
while time.monotonic() < deadline:
    ...
    if queue.get("cancelled"):
        raise SystemExit("Jenkins cancelled the portfolio-java deployment while queued.")
    if queue.get("executable"):
        build_number = queue["executable"]["number"]
        break
    time.sleep(2)
if build_number is None:
    raise SystemExit("Timed out waiting for Jenkins to assign the portfolio-java deployment job.")

print(f"portfolio-java build assigned: #{build_number}")
build_api = f"{base_url}/job/{job_path}/{build_number}/api/json"
while time.monotonic() < deadline:
    _, build = request_json(build_api)
    if not build.get("building", False):
        result = build.get("result")
        print(f"portfolio-java build result: {result}")
        if result != "SUCCESS":
            ...
            raise SystemExit(f"portfolio-java Jenkins build failed with result {result}.")
        raise SystemExit(0)
    time.sleep(3)
raise SystemExit("Timed out waiting for portfolio-java Jenkins build to finish.")
PY
