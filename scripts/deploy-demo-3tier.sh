...
JENKINS_DEMO_JOB=${JENKINS_DEMO_JOB:-demo-3tier-deploy}
JENKINS_BUILD_TIMEOUT_SECONDS=${JENKINS_DEMO_BUILD_TIMEOUT_SECONDS:-1800}
...
LOCAL_AUTH_FILE=$(mktemp /tmp/private-banking-jenkins-demo-auth.XXXXXX)
KNOWN_HOSTS=$(mktemp /tmp/private-banking-jenkins-demo-known-hosts.XXXXXX)
REMOTE_AUTH_FILE="/tmp/private-banking-jenkins-demo-auth.$$"
cleanup() {
...
while time.monotonic() < deadline:
    ...
    if queue.get("cancelled"):
        raise SystemExit("Jenkins cancelled the demo deployment while queued.")
    if queue.get("executable"):
        build_number = queue["executable"]["number"]
        break
    time.sleep(2)
if build_number is None:
    raise SystemExit("Timed out waiting for Jenkins to assign the demo deployment job.")

print(f"demo-3tier build assigned: #{build_number}")
build_api = f"{base_url}/job/{job_path}/{build_number}/api/json"
while time.monotonic() < deadline:
    _, build = request_json(build_api)
    if not build.get("building", False):
        result = build.get("result")
        print(f"demo-3tier build result: {result}")
        if result != "SUCCESS":
            ...
            raise SystemExit(f"demo-3tier Jenkins build failed with result {result}.")
        raise SystemExit(0)
    time.sleep(3)
raise SystemExit("Timed out waiting for demo-3tier Jenkins build to finish.")
PY
