...
JENKINS_SMOKE_JOB=${JENKINS_SMOKE_JOB:-platform-worker-smoke}
JENKINS_BUILD_TIMEOUT_SECONDS=${JENKINS_BUILD_TIMEOUT_SECONDS:-600}
WORKLOAD_KEY=/home/ubuntu/.ssh/private-banking-openstack-workloads

for binary in aws python3 ssh scp; do
...
if [[ -z "$JENKINS_FLOATING_IP" ]]; then
  printf 'Discovering Jenkins controller floating IP...\n'
  JENKINS_FLOATING_IP=$(
...
LOCAL_AUTH_FILE=$(mktemp /tmp/private-banking-jenkins-cli-auth.XXXXXX)
KNOWN_HOSTS=$(mktemp /tmp/private-banking-jenkins-smoke-known-hosts.XXXXXX)
REMOTE_AUTH_FILE="/tmp/private-banking-jenkins-smoke-auth.$$"
...
cleanup() {
  ssh \
    ...
    "rm -f '$REMOTE_AUTH_FILE'" >/dev/null 2>&1 || true
...
scp "${SSH_OPTS[@]}" \
  "$LOCAL_AUTH_FILE" \
  "ubuntu@${JENKINS_FLOATING_IP}:${REMOTE_AUTH_FILE}"

ssh "${SSH_OPTS[@]}" "ubuntu@${JENKINS_FLOATING_IP}" \
  "chmod 0600 '$REMOTE_AUTH_FILE'"

printf 'Triggering Jenkins worker smoke job %s through the controller local API...\n' "$JENKINS_SMOKE_JOB"
...
try:
    with urllib.request.urlopen(build_request, timeout=15) as response:
        queue_url = response.headers.get("Location")
except urllib.error.HTTPError as exc:
    detail = exc.read().decode(errors="replace")
    raise SystemExit(f"Unable to trigger Jenkins job: HTTP {exc.code}: {detail}")
...
while time.monotonic() < deadline:
    ...
    if queue.get("cancelled"):
        raise SystemExit("Jenkins cancelled the smoke job while it was queued.")
...
if build_number is None:
    raise SystemExit("Timed out waiting for Jenkins to assign the smoke job to the worker executor.")

print(f"Smoke build assigned: #{build_number}")
...
while time.monotonic() < deadline:
    ...
    if not build.get("building", False):
        ...
        print(f"Smoke build result: {result}")
        for artifact in artifacts:
            print(f"Archived artifact: {artifact.get('relativePath')}")
        if result != "SUCCESS":
            raise SystemExit(f"Jenkins worker smoke build failed with result {result}.")
        if not any(str(item.get("fileName", "")).endswith(".jar") for item in artifacts):
            raise SystemExit("Smoke build succeeded but no Java .jar artifact was archived.")
        print("Jenkins worker smoke validation: SUCCESS")
        raise SystemExit(0)
    time.sleep(3)

raise SystemExit("Timed out waiting for the Jenkins worker smoke build to finish.")
PY
